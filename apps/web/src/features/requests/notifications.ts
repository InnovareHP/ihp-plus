...
import { portalUrl, requestDecidedTemplate, requestSubmittedTemplate, sendEmail } from '@/lib/email'
...
export async function notifyApprovers(request: SubmittedRequest) {
  try {
    ...
    const recipientIds = [...new Set(candidateIds)].filter(
      (userId) => userId !== request.requesterId,
    )
